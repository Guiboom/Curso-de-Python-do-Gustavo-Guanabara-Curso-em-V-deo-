...
while True:
    try:
        ...
        cadastro["ctps"] = int(
            input("Número da carteira de trabalho (0 caso não tenha): ")
        )

        if cadastro["ctps"] != 0:
            cadastro["contratação"] = int(input("Ano de contratação: "))
            cadastro["idade para aposentadoria"] = (
                cadastro["idade"] + 35 - (ano_atual - cadastro["contratação"])
            )
        ...
        if cadastro["ctps"] != 0:
            ...
            if cadastro["idade para aposentadoria"] > 0:
                print(
                    f'Status: Você deve se aposentar aos {cadastro["idade para aposentadoria"]} anos'
                )
            else:
